...
public class ThirdPersonControllerNET : MonoBehaviour
{
	...
	//New Variables 
	public static bool strikeBool = false;
	...
	*/
	...
	void Strike () 
	{
		StartCoroutine(StrikeTime());
	}
	
	
	
	IEnumerator StrikeTime()
	{
		GameObject strikeBox = this.gameObject.transform.GetChild(1).gameObject;
        ...
        //Strike is performed 
        blocking.currentBlockStamina -= 100;
        if (blocking.currentBlockStamina <= 0)
            blocking.currentBlockStamina = 0;

		strikeBox.SetActive(true);
        playerHealth.playerAudio.clip = playerHealth.swingingClip;
        playerHealth.playerAudio.Play();

		yield return new WaitForSeconds(.5f);
		strikeBox.SetActive(false);
	...
	{ 
			Debug.Log ("Update kills method");
			playerHealth.UpdateKills();
	}
	
	void OnTriggerEnter(Collider myTrigger){
 
		if(myTrigger.gameObject.tag == "Strike")
  		{
            if (blocking.block)
            {
                playerHealth.playerAudio.clip = playerHealth.defendingClip;
                playerHealth.playerAudio.Play();

                Rigidbody rigidbody = this.gameObject.GetComponent<Rigidbody>();
                rigidbody.AddForce(-transform.forward * 100);
                //Rigidbody otherRigidbody = myTrigger.gameObject.transform.parent.GetComponent<Rigidbody>();
                // otherRigidbody.AddForce(-transform.forward * 1000);
            }
            else 
            {
                playerHealth.playerAudio.clip = playerHealth.strikingClip;
                playerHealth.playerAudio.Play();

                strikeObject = myTrigger.gameObject.transform.parent.gameObject;
				//int strikeObjectID = strikeObject.ThirdPersonNetworkVik.playerID;
				//Debug.Log (strikeObjectID);
				//Debug.Log ("Hit");
	...
	// Handle rotation here to ensure smooth application.
	{
		...
		if (Input.GetMouseButton (0))
		{
			...
			if (strikeBool == false)
			{
                if (blocking.currentBlockStamina >= 90)
                {
					strikeBool = true;
                    Strike();
                }
		...
		{
			if(blocking.currentBlockStamina > 0) //recharge stops you being able to use stamina when recharging
			{
				gameObject.transform.GetChild(2).gameObject.SetActive(true);	
				blocking.block = true;
				blocking.currentBlockStamina -= 1f; //take away stamina
			}
			else
			{
		...
		else
		{
			...
			{
				blocking.currentBlockStamina += 1f;
			}
		}
		
	}
	
	//Viking Demo Movement 
		/*
